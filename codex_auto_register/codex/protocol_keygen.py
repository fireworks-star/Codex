...
IMAP_SERVER = _config.get("imap_server", "imap.qq.com")
...
def create_temp_email(session):
    ...
    if CUSTOM_DOMAIN:
        ...
        email = f"{username}@{CUSTOM_DOMAIN.lstrip('@')}"
        print(f"  ✅ 使用自定义域名邮箱: {email}")
        return email, "CUSTOM_IMAP_TOKEN"
...
def wait_for_verification_code(session, email_address, dm_token, timeout=300, start_time=None):
    ...
    # 降级走 DuckMail 逻辑
    old_ids = set()
    old = fetch_emails(session, email_address, dm_token)
    if old:
    ...
    if page_type == "email_otp_verification" or "email-verification" in continue_url:
        ...
        if code:
            ...
            if resp.status_code == 200:
                ...
                print(f"  ❌ 验证码 {code} 失败: {resp.status_code}")
                code = None
